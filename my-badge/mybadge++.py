...
Freelance:D
GitHub|Hackclub
Hackclubber|GithubNerd|IdkAfterThis
he/him
...
def draw_badge():
    display.set_pen(15)
    display.clear()
    TEXT_WIDTH = WIDTH - LEFT_PADDING - 2
    # Draw the background
  #  try:
  ...
  #      print("Badge background error")
    ...
    while True:
        name_length = display.measure_text(first_name.split("|")[0], name_size)
        if name_length >= TEXT_WIDTH and name_size >= 0.1:
            name_size -= 0.01
        else:
            display.text(first_name.split("|")[0], LEFT_PADDING, 5 + vertical_adjustment, TEXT_WIDTH, name_size)
            break
    ...
    while True:
        ...
        if lastname_length >= TEXT_WIDTH and lastname_size >= 0.1:
            ...
            display.text(last_name.split("|")[0], LEFT_PADDING, NAME_HEIGHT + LINE_SPACING + vertical_adjustment, TEXT_WIDTH, lastname_size)
            break
    ...
    # Title
    print(index_for_title)
    print("####index_for_title setting")
    display.text(title.split("|")[index_for_title], LEFT_PADDING, HEIGHT - (DETAILS_HEIGHT * 2) - LINE_SPACING - 2, TEXT_WIDTH, DETAILS_TEXT_SIZE * size_adjustment)
    ...
    if pronouns and pronouns.strip() != "":
        display.text(pronouns.split("|")[0], LEFT_PADDING, HEIGHT - DETAILS_HEIGHT, TEXT_WIDTH, DETAILS_TEXT_SIZE * size_adjustment)
    else:
        display.text(handle.split("|")[0], LEFT_PADDING, HEIGHT - DETAILS_HEIGHT, TEXT_WIDTH, DETAILS_TEXT_SIZE * size_adjustment)
...
try:
    event = badge.split("\n")[0]         # "Universe 2024"
    first_name = badge.split("\n")[1]    # "Mona Lisa"
    last_name = badge.split("\n")[2]     # "Octocat"
    company = badge.split("\n")[3]       # "GitHub"
    title = badge.split("\n")[4]         # "Company Mascot"
    pronouns = badge.split("\n")[5]      # "she/her"
    handle = badge.split("\n")[6]        # "@mona"
...
while True:
    index_till_switch += 1
    print(index_till_switch)
    if index_till_switch > 150:
        index_for_title = random.randint(0,len(title.split("|")) - 1)
        index_till_switch = 0
        changed = True
